...
> 注意：本仓库的源码已经做过几轮重构（例如：`POST /demo/generate` 改为“直接流式返回 Parquet”，不再在本地落地 `./data/demo.parquet`；`GET /demo/download` 改为“模拟 S3/GCS → 本地临时 parquet → 导出”，并将 Controller 尽量做薄）。
> 本文档中的某些代码片段/路径仅作思路参考，**以当前仓库源码为准**。
...
public class DemoController {
  ...
  public Mono<Void> download(@RequestParam(defaultValue = "zip") FileFormat format,
    ...
    String parquetFilename = parquet.getFileName().toString();
    String baseName = parquetFilename.endsWith(".parquet")
        ? parquetFilename.substring(0, parquetFilename.length() - ".parquet".length())
        : parquetFilename;
  ...
  private void writeZipCsvTo(OutputStream rawOut, java.nio.file.Path parquetFile, long maxRows) {
    try {
      ...
      try (ZipOutputStream zos = new ZipOutputStream(bos, CSV_CHARSET)) {
        ...
        String parquetFilename = parquetFile.getFileName().toString();
        String baseName = parquetFilename.endsWith(".parquet")
            ? parquetFilename.substring(0, parquetFilename.length() - ".parquet".length())
            : parquetFilename;
